      ...
      <div className="min-h-screen">
        ...
        <div className="space-y-4 mb-4">
          {ordersData.data?.map((order, index) => {
            // Safely fetch primary dynamic line items from array
            const primeItem = order.orderDetails?.[0];
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {primeItem && (
                          <span className="px-2 py-1 bg-gray-100 rounded text-xs font-medium text-gray-600">
                            {primeItem.name || primeItem.ingredientId?.name} (x{primeItem.quantity})
                          </span>
                        )}
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-6 min-w-[200px]">
                    ...
                    <button
                      onClick={() =>
                        router.push(`/vendor/ingredients/my-orders/${order.orderId}`)
                      }
                      className="p-2 text-gray-400 hover:text-[#DC3173] hover:bg-[#DC3173]/10 rounded-full transition-all cursor-pointer"
                    >
        ...
        {!!ordersData?.meta?.totalPage && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <PaginationComponent totalPages={ordersData?.meta?.totalPage as number} />
          </motion.div>
        )}
      </div>
    </div>
