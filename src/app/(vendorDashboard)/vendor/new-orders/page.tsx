...
import { Clock, MapPin, ChevronRight,  CookingPot, CheckCircle, Circle, Truck } from "lucide-react";
...
export default function NewOrdersPage() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const pick = incomingPool[Math.floor(Math.random() * incomingPool.length)];
      ...
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
          <h1
            ...
            style={{ backgroundImage: `linear-gradient(90deg, ${PRIMARY}, #ff5fa2)` }}
          >
            Vendor — New Orders
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <Card className="lg:col-span-3 bg-white shadow-xl rounded-2xl border border-pink-200">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-xl font-bold" style={{ color: PRIMARY }}>
                <span className="w-3 h-3 rounded-full animate-pulse" style={{ background: PRIMARY }} />
                Live Orders
              </CardTitle>
            </CardHeader>
                  ...
                  >
                    <div className="flex justify-between gap-4">
                      ...
                      <div className="flex gap-4 overflow-hidden min-w-0">
                        <div
                          className="w-14 h-14 rounded-xl flex items-center justify-center text-2xl font-bold shadow-sm flex-shrink-0"
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <h3 className="text-lg font-semibold text-gray-800 truncate">{order.customer}</h3>
                            <span className="text-xs text-gray-500 flex items-center gap-1">
                              <Clock size={12} /> {order.time}
                            </span>
                          </div>

                          <div className="mt-2 text-sm text-gray-700 leading-5 break-words">
                            {order.items.map((item, i) => (
                              <span key={i} className="mr-2 inline-block truncate">
                                {item.qty}× {item.name}
                              </span>
                            ))}
                          </div>
                      ...
                      <div className="flex flex-col items-end justify-between min-w-[180px]">
                        ...
                        <div className="flex items-center gap-2 mt-3">
                          ...
                          <Sheet>
                            ...
                            <SheetContent side="right" className="w-full md:w-[460px] p-6 overflow-y-auto">
                              <OrderDetails
                                order={order}
...
<Card className="p-6 bg-white shadow-xl rounded-2xl border border-pink-200">
  <h2 className="text-xl font-extrabold mb-6" style={{ color: PRIMARY }}>
    Overview
  </h2>
    ...
    />
            ...
            <Card className="p-5 rounded-2xl bg-white shadow">
              ...
              <CardContent className="flex items-center gap-3">
                <div className="p-3 rounded-full" style={{ background: PINK_SOFT }}>
                  <Truck />
                </div>
                <div>
                  <p className="text-sm text-gray-600">Delivery partners online</p>
...
function StatusBadge({ status }: { status: Status }) {
  ...
  const m = map[status];
  return (
    <div className="px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-2" style={{ background: m.bg, color: m.color }}>
        ...
        return (
          <div key={s.key} className="flex items-center gap-2">
            <motion.div
              ...
              animate={isActive ? { scale: 1.05, opacity: 1, boxShadow: `0 6px 18px ${PRIMARY}33` } : { scale: 1, opacity: 0.6 }}
              transition={{ type: "spring", stiffness: 160, damping: 16 }}
              className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold ${isActive ? "bg-[--primary] text-white" : "bg-white border border-pink-100 text-rose-500"}`}
              style={isActive ? { background: PRIMARY, color: "#fff" } : {}}
            >
              {i + 1}
            </motion.div>

            {i < steps.length - 1 && <div className={`h-0.5 w-8 ${i < active ? "bg-[--primary]" : "bg-pink-100"}`} style={i < active ? { background: PRIMARY } : undefined} />}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <Badge className="border-[--brand]" style={{ borderColor: PRIMARY, color: PRIMARY }}>{order.eta}</Badge>
      ...
      <div>
        ...
        <ul className="mt-2 space-y-2">
          {order.items.map((it, idx) => (
            <li key={idx} className="flex items-center justify-between p-3 rounded-md bg-gray-50">
      ...
      <div>
        ...
        <p className="text-sm text-gray-600 mt-1 flex items-center gap-2"><MapPin size={14} /> {order.address}</p>
      </div>

      <div className="flex items-center gap-2 justify-end">
        <Button variant="ghost" onClick={() => onAccept && onAccept()}>Accept</Button>
        <Button variant="outline" onClick={() => onStart && onStart()}>Start</Button>
        <Button variant="destructive" onClick={() => onReady && onReady()}>Ready</Button>
      </div>
    </div>
  );
}
